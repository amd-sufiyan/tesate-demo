...
class Bootstrap_Comment_Walker extends Walker_Comment {
	...
	protected function html5_comment( $comment, $depth, $args ) {
		...
		<<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $this->has_children ? 'has-children media' : ' media' ); ?>>
			...
			<div class="media-body card mt-3 card-comment" id="div-comment-<?php comment_ID(); ?>">
				<div class="card-body" style="border-left:3px solid red">
					...
					<div class="comment-metadata flex-center">
						<a class="hidden-xs-down" href="<?php echo esc_url( get_comment_link( $comment->comment_ID, $args ) ); ?>">
							<time class=" small btn btn-link chip" datetime="<?php comment_time( 'c' ); ?>">
								<?php comment_date() ?>,
								<?php comment_time() ?>
							</time>
						</a>
						<ul class="list-inline">
							<?php edit_comment_link( __( 'Edit' ), '<li class="edit-link list-inline-item btn btn-sm ">', '</li>' ); ?>
							<?php
								comment_reply_link( array_merge( $args, array(
									'add_below' => 'div-comment',
									'depth'     => $depth,
									'max_depth' => $args['max_depth'],
									'before'    => '<li class=" reply-link list-inline-item btn btn-sm btn-warning">',
									'after'     => '</li>'
								) ) );	
							?>
						</ul>
